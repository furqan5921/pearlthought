...
export function ProgressSection() {
  ...
  const averageRating = teachers.length > 0 
    ? teachers.reduce((sum, t) => sum + (t.performanceRating || 0), 0) / teachers.length 
    : 0;
  
  const excellentPerformers = teachers.filter(t => (t.performanceRating || 0) >= 4.5).length;
  const goodPerformers = teachers.filter(t => (t.performanceRating || 0) >= 4.0 && (t.performanceRating || 0) < 4.5).length;
  const needsImprovement = teachers.filter(t => (t.performanceRating || 0) < 4.0).length;
  ...
  const filteredTeachers = teachers.filter(teacher => {
    ...
    if (performanceFilter === 'excellent') {
      ...
      matchesPerformance = (teacher.performanceRating || 0) >= 4.0 && (teacher.performanceRating || 0) < 4.5;
  ...
  const departmentPerformance = departments.map(dept => {
    ...
    const avgRating = deptTeachers.length > 0 
      ? deptTeachers.reduce((sum, t) => sum + (t.performanceRating || 0), 0) / deptTeachers.length 
      : 0;
    return {
      department: dept,
  ...
  const getPerformanceBadge = (rating: number) => {
    if (rating >= 4.5) return { variant: 'default' as const, label: 'Excellent', color: 'text-green-600' };
    if (rating >= 4.0) return { variant: 'secondary' as const, label: 'Good', color: 'text-blue-600' };
    if (rating >= 3.5) return { variant: 'outline' as const, label: 'Average', color: 'text-yellow-600' };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 truncate">Teacher Progress</h1>
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Track performance metrics and professional development</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{goodPerformers}</div>
            <p className="text-xs text-muted-foreground">
              4.0-4.4 rating
            </p>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Overall performance trend over the past 7 months</CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={250}>
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            {filteredTeachers.map((teacher) => {
              ...
              return (
                <div key={teacher.id} className="flex flex-col lg:flex-row lg:items-center lg:justify-between p-4 border rounded-lg hover:bg-gray-50 gap-4">
                  <div className="flex items-center space-x-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs sm:text-sm text-gray-500 truncate">
                        {teacher.employeeId} • {teacher.department} • {teacher.experience} years exp.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                    <div className="grid grid-cols-3 sm:flex sm:items-center gap-4 sm:gap-6">
                      <div className="text-center">
                        <p className="text-xs sm:text-sm text-gray-500">Current Rating</p>
                        <div className="flex items-center justify-center space-x-1">
                          <Star className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-500 fill-current" />
                          <span className="font-semibold text-sm sm:text-base">{(teacher.performanceRating || 0).toFixed(1)}</span>
                        </div>
                      </div>
                      <div className="text-center">
                        <p className="text-xs sm:text-sm text-gray-500">Last Review</p>
                        <p className="font-medium text-xs sm:text-sm">
                          {teacher.lastReviewDate ? new Date(teacher.lastReviewDate).toLocaleDateString() : 'N/A'}
                        </p>
                      </div>
                      <div className="text-center">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Active professional development objectives</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                <p className="font-medium">Improve Student Engagement</p>
                <p className="text-sm text-gray-500">Target: 4.8/5.0 by Dec 2024</p>
              </div>
            ...
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                ...
                <p className="text-sm text-gray-500">Complete digital tools training</p>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Notable accomplishments and recognitions</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center space-x-3 p-3 border rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Sarah Johnson - July 2024</p>
              </div>
            </div>
            <div className="flex items-center space-x-3 p-3 border rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Advanced Teaching Methods - 5 teachers</p>
              </div>
            </div>
            <div className="flex items-center space-x-3 p-3 border rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Department average above 4.5</p>
              </div>
            </div>
          </CardContent>
        </Card>
