...
export function TeachersSection() {
  const { teachers, getFilteredTeachers, deleteTeacher } = useTeacherStore();
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 truncate">Teachers</h1>
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage your teaching staff and their information</p>
        </div>
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-2xl mx-4">
            ...
            <div className="p-4">
              <p className="text-sm text-gray-500">Teacher form would go here...</p>
            </div>
          </DialogContent>
        </Dialog>
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 truncate">{teacher.employeeId}</p>
                  </div>
                </div>
                <Badge
                  variant={teacher.status === 'active' ? 'default' : teacher.status === 'on-leave' ? 'secondary' : 'destructive'}
                ...
                <TabsContent value="personal" className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm">{new Date(selectedTeacher.dateOfBirth).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-500">Hire Date</label>
                      <p className="text-sm">{new Date(selectedTeacher.hireDate).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-500">Employment Type</label>
                      <p className="text-sm capitalize">{selectedTeacher.employmentType}</p>
                    </div>
                    <div>
                ...
                <TabsContent value="professional" className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">${selectedTeacher.salary.toLocaleString()}</p>
                    </div>
                  </div>
                  <div>
                ...
                <TabsContent value="contact" className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedTeacher.address.street}<br />
                        {selectedTeacher.address.city}, {selectedTeacher.address.state} {selectedTeacher.address.zipCode}<br />
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedTeacher.emergencyContact.name} ({selectedTeacher.emergencyContact.relationship})<br />
                      ...
                      <div key={doc.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 border rounded-lg">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            {doc.type} • {(doc.size / 1024).toFixed(0)} KB • {new Date(doc.uploadDate).toLocaleDateString()}
                          </p>
                        </div>
