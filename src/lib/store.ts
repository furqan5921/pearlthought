      ...
      (set, get) => ({
        ...
        getFilteredTeachers: () => {
          ...
          return teachers.filter(teacher => {
            const matchesSearch = 
              teacher.firstName.toLowerCase().includes(filters.search.toLowerCase()) ||
              teacher.lastName.toLowerCase().includes(filters.search.toLowerCase()) ||
            ...
            const matchesEmploymentType = filters.employmentType === 'all' || teacher.employmentType === filters.employmentType;
            const matchesExperience = teacher.experience >= filters.experience.min && teacher.experience <= filters.experience.max;
            const matchesSalary = teacher.salary >= filters.salary.min && teacher.salary <= filters.salary.max;

            return matchesSearch && matchesDepartment && matchesStatus && matchesEmploymentType && matchesExperience && matchesSalary;
          });
        },
  ...
  devtools((set, get) => ({
    ...
    refreshDashboard: () => {
      ...
      const metrics: DashboardMetrics = {
        ...
        activeTeachers: teacherStore.teachers.filter(t => t.status === 'active').length,
        attendanceRate: attendanceStore.getAttendanceRate(),
        averageSalary: teacherStore.teachers.reduce((sum, t) => sum + t.salary, 0) / teacherStore.teachers.length || 0,
